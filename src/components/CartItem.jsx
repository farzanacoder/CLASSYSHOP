...
const CartItem = ({subtitle , title , price , ShpBrnd , className}) => {
  return (
    ...
    <Flex className={`${className} pl-3.5 absolute top-44 h-[200px] flex-col text-left gap-1 items-start justify-center`}>
      ...
      <h1 className="text-sm mb-2 w-full font-mont font-semibold hover:text-red duration-300 cursor-pointer">{subtitle}</h1>
      ...
      <button className='font-semibold border rounded-md py-2 px-9 text-red hover:text-white hover:bg-black hover:border-black overflow-hidden uppercase text-sm cursor-pointer duration-300'>add to card</button>
    </Flex>
    </div>
  )
}
